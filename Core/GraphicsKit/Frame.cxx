...
namespace Cyanite::GraphicsKit {
    Frame::Frame(ID3D12Device* device, ID3D12PipelineState* pso, ID3D12PipelineState* shadowMapPso, ID3D12DescriptorHeap* dsvHeap, ID3D12DescriptorHeap* cbvSrvHeap, D3D12_VIEWPORT* viewport, UINT frameResourceIndex) :
        _fenceValue(0),
        _pipelineState(pPso),
        _pipelineStateShadowMap(pShadowMapPso)
    {
    ...
    // this frame resource.
    void Frame::WriteConstantBuffers(D3D12_VIEWPORT* pViewport, Camera* pSceneCamera, Camera lightCams[NumLights], LightState lights[NumLights])
    {
